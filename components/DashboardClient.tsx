...
import { getUserDashboardWidgets } from '../lib/dashboard';
...
export function DashboardClient() {
  ...
  // Get user widgets
  const userWidgets = getUserDashboardWidgets();
        ...
        {loading && !accounts.length && !transactions.length ? (
          ...
          <>
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              {userWidgets.filter(w => w.visible && w.type === 'recent-transactions').length > 0 && (
              ...
              {userWidgets.filter(w => w.visible && w.type === 'top-categories').length > 0 && (
